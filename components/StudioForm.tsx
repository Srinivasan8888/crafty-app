...
export function StudioForm({ cities, disciplines, crafters, entityId, initialValues }: Props) {
  ...
  const unmet: string[] = [];
  if (form.name.trim().length < 3) unmet.push("Studio name needs 3+ characters");
  if (!form.logo_photo) unmet.push("Add a logo / studio photo");
  if (!form.is_online_only && form.address.trim().length === 0) unmet.push("Add an address");
  if (form.discipline_ids.length === 0) unmet.push("Pick at least one discipline");
  if (
    !(
      (form.contact_phone.trim() && looksLikePhone(form.contact_phone)) ||
      (form.contact_whatsapp.trim() && looksLikePhone(form.contact_whatsapp)) ||
        ...
        <div className="space-y-3">
          <div>
            <input
              ...
              aria-describedby={errors.contact_phone ? "studio-phone-error" : undefined}
            />
            {errors.contact_phone && (
              <p id="studio-phone-error" role="alert" className="mt-1 text-sm text-danger">
                {errors.contact_phone}
              </p>
            )}
          </div>
          <div>
            <input
              ...
              aria-describedby={errors.contact_whatsapp ? "studio-whatsapp-error" : undefined}
            />
            {errors.contact_whatsapp && (
              <p id="studio-whatsapp-error" role="alert" className="mt-1 text-sm text-danger">
                {errors.contact_whatsapp}
              </p>
            )}
          </div>
          <div>
            <input
              ...
              aria-describedby={errors.contact_website ? "studio-website-error" : undefined}
            />
            {errors.contact_website && (
              <p id="studio-website-error" role="alert" className="mt-1 text-sm text-danger">
                {errors.contact_website}
              </p>
            )}
          </div>
