...
const PatchSchema = z.object({
  ...
  category_ids: z.array(z.string().max(30)).min(1).max(5).optional(),
  // storefront-completeness — crafters this store sources/stocks. Empty array
...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (!rl.allowed) return NextResponse.json({ error: "rate_limited" }, { status: 429, headers: { "Retry-After": String(Math.ceil(rl.resetIn / 1000)) } });
  ...
  if (data.category_ids) {
    const ok = await prisma.supplyCategory.count({ where: { id: { in: data.category_ids }, is_active: true } });
  ...
  if (data.name && data.name !== existing.name) {
    newSlug = await ensureUniqueSlug(data.name, async (s) =>
      !!(await prisma.store.findFirst({ where: { slug: s, NOT: { id: existing.id } } })),
    );
    if (newSlug !== existing.slug) {
      await prisma.slugRedirect.upsert({
        where: { entity_type_old_slug: { entity_type: "store", old_slug: existing.slug } },
        create: { entity_type: "store", old_slug: existing.slug, new_slug: newSlug },
  ...
  revalidatePath(`/${updated.city.slug}/stores`);
  revalidatePath(`/${updated.city.slug}/stores/${updated.slug}`);
  if (newSlug !== existing.slug) {
    revalidatePath(`/${existing.city.slug}/stores/${existing.slug}`);
  }
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  revalidatePath(`/${existing.city.slug}/stores`);
  revalidatePath(`/${existing.city.slug}/stores/${existing.slug}`);
