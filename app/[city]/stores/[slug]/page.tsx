...
const loadStore = cache(async (citySlug: string, slug: string) => {
  ...
  if (!store || store.status !== "PUBLISHED" || store.city_id !== city.id) {
    const redirected = await prisma.slugRedirect.findUnique({
      where: { entity_type_old_slug: { entity_type: "store", old_slug: slug } },
    }).catch(() => null);
    if (redirected) {
      redirect(`/${citySlug}/stores/${redirected.new_slug}`);
    }
    return null;
  }
  return store;
