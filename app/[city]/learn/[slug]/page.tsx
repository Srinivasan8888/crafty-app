...
export default async function StudioDetail({ params }: { params: { city: string; slug: string } }) {
  ...
  // storefront-completeness — crafters the owner tagged as teaching here win
  // over the generic city sample. Same select shape so the render loop is shared.
  ...
  const [taggedCrafters, sampleCrafters] = await Promise.all([
    prisma.crafter.findMany({
      where: { status: "PUBLISHED", tagged_in_studios: { some: { studio_id: s.id } } },
          ...
          <div className="lg:col-span-2">
            ...
            <section className="detail-section" style={{ borderTop: "1px solid var(--line)", paddingTop: 24, marginTop: 24 }}>
              <h2 style={{ fontSize: 22, marginBottom: 14 }}>
                {hasTagged ? "Crafters who teach here" : `Crafters in ${s.city.display_name}`}
              </h2>
